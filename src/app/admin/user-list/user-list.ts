...
onScroll(): void {
  ...
  if (
    scrollPosition >= pageHeight - 200 &&  !this.loadingMore &&   !this.loading && this.hasMoreUsers) {
    this.loadMoreUsers();
  }
}
loadUsers(){
...
changeUserRole(user: any){
  ...
  this.dialogService.openConfirmation(
    'Change User Role?',
    `Are you sure you want to change the role of user ${user.email} to ${newRole}?`,
    'Change Role',
'Cancel',
'warning').subscribe(response => {
      if(response){
